try:
    from tensorflow.keras.layers import Input, concatenate
    from tensorflow.keras.layers import Conv3D, MaxPooling3D, UpSampling3D, BatchNormalization
    from tensorflow.keras.models import Model
except ImportError:
    pass

def resblock(x, filter=24):
    res=x
    x=Conv3D(filter, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    x=Conv3D(filter, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    x+=res
    return x


def my_model(dim_in, Ncl):
    input=Input(shape=(dim_in,dim_in,dim_in,1))

    x=Conv3D(24, (3,3,3), padding='same', activation='relu')(input)
    x=BatchNormalization()(x)

    high=resblock(x, filter=24)

    x = MaxPooling3D((2,2,2), strides=None)(high)
    x=Conv3D(48, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    mid1=resblock(x, filter=48)

    x = MaxPooling3D((2,2,2), strides=None)(mid1)
    x=Conv3D(72, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    mid2=resblock(x,filter=72)

    x = MaxPooling3D((2,2,2), strides=None)(mid2)
    x=Conv3D(108, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    x=Conv3D(108, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    x=Conv3D(108, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)


    x = UpSampling3D(size=(2,2,2), data_format='channels_last')(x)
    x = Conv3D(72, (2,2,2), padding='same', activation='relu')(x)
    x=concatenate([x, mid2])
    x = Conv3D(72, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    x=resblock(x, filter=72)

    x = UpSampling3D(size=(2,2,2), data_format='channels_last')(x)
    x = Conv3D(48, (2,2,2), padding='same', activation='relu')(x)
    x=concatenate([x, mid1])
    x = Conv3D(48, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    x=resblock(x, filter=48)

    x = UpSampling3D(size=(2,2,2), data_format='channels_last')(x)
    x = Conv3D(24, (2,2,2), padding='same', activation='relu')(x)
    x=concatenate([x, high])
    x = Conv3D(24, (3,3,3), padding='same', activation='relu')(x)
    x=BatchNormalization()(x)

    x=resblock(x, filter=24)

    output = Conv3D(Ncl, (1,1,1), padding='same', activation='softmax')(x)
    model = Model(input, output)
    return model







