...
def ensure_model_exists(model_name):
    ...
    # Read cfg file from pretrained folder
    cfg_file = os.path.join(pretrained_path, 'model_paths.cfg')
    ...
    # Check if the model path is provided in the cfg file
    if config.has_section('Models') and config.has_option('Models', model_name):
        model_filepath = config.get('Models', model_name)
        # Check if the specified path exists
        if not os.path.isfile(model_filepath):
            print(f"Model path provided in cfg file does not exist: {model_filepath}")
            model_filepath = None
        else:
            print(f"Using model path from cfg file: {model_filepath}")

    # If no valid path in cfg file, check locally or download
    if model_filepath is None:
        ...
        if not os.path.isfile(model_filepath):
            print(f"{model_name} does not exist locally, downloading from the remote server...")
            ...
            print(f"{model_name} already exists locally, path: {model_filepath}")
